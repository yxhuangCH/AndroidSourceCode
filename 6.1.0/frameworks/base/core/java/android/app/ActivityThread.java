...
public final class ActivityThread {
    ...
    static final class Profiler {
        ...
        public void startProfiling() {
            ...
            try {
                VMDebug.startMethodTracing(profileFile, profileFd.getFileDescriptor(), 8 * 1024 * 1024, 0,
    ...
    private class ApplicationThread extends ApplicationThreadNative {
        ...
        public final void bindApplication(String processName, ApplicationInfo appInfo, List<ProviderInfo> providers,
            ...
            /*
             * Two possible indications that this package could be sharing its runtime with
             * other packages:
             *
             * 1.) the sharedUserId attribute is set in the manifest, indicating a request
             * to share a VM with other packages with the same sharedUserId.
             *
             * 2.) the application element of the manifest has an attribute specifying a
             * non-default process name, indicating the desire to run in another packages
             * VM.
             *
             * If sharing is enabled we do not have a unique application in a process and
             * therefore cannot rely on the package name inside the runtime.
             */
        ...
        public void dumpMemInfo(FileDescriptor fd, Debug.MemoryInfo mem, boolean checkin, boolean dumpFullInfo,
                boolean dumpDalvik, boolean dumpSummaryOnly, String[] args) {
    ...
    Resources getTopLevelResources(String resDir, String[] splitResDirs, String[] overlayDirs, String[] libDirs,
            int displayId, Configuration overrideConfiguration, LoadedApk pkgInfo) {
        return mResourcesManager.getTopLevelResources(resDir, splitResDirs, overlayDirs, libDirs, displayId,
    ...
    public static void dumpMemInfoTable(PrintWriter pw, Debug.MemoryInfo memInfo, boolean checkin, boolean dumpFullInfo,
        ...
        if (!dumpSummaryOnly) {
            if (dumpFullInfo) {
                printRow(pw, HEAP_FULL_COLUMN, "", "Pss", "Pss", "Shared", "Private", "Shared", "Private", "Swapped",
                        "Heap", "Heap", "Heap");
                ...
                printRow(pw, HEAP_FULL_COLUMN, "Native Heap", memInfo.nativePss, memInfo.nativeSwappablePss,
                        ...
                        memInfo.nativePrivateClean, memInfo.nativeSwappedOut, nativeMax, nativeAllocated, nativeFree);
                printRow(pw, HEAP_FULL_COLUMN, "Dalvik Heap", memInfo.dalvikPss, memInfo.dalvikSwappablePss,
                        ...
                        memInfo.dalvikPrivateClean, memInfo.dalvikSwappedOut, dalvikMax, dalvikAllocated, dalvikFree);
            } else {
                ...
                if (myPss != 0 || mySharedDirty != 0 || myPrivateDirty != 0 || mySharedClean != 0 || myPrivateClean != 0
                    ...
                    if (dumpFullInfo) {
                        printRow(pw, HEAP_FULL_COLUMN, Debug.MemoryInfo.getOtherLabel(i), myPss, mySwappablePss,
                                mySharedDirty, myPrivateDirty, mySharedClean, myPrivateClean, mySwappedOut, "", "", "");
                    } else {
            ...
            if (dumpFullInfo) {
                printRow(pw, HEAP_FULL_COLUMN, "Unknown", otherPss, otherSwappablePss, otherSharedDirty,
                        otherPrivateDirty, otherSharedClean, otherPrivateClean, otherSwappedOut, "", "", "");
                printRow(pw, HEAP_FULL_COLUMN, "TOTAL", memInfo.getTotalPss(), memInfo.getTotalSwappablePss(),
                        ...
                        memInfo.getTotalPrivateClean(), memInfo.getTotalSwappedOut(), nativeMax + dalvikMax,
        ...
        if (r != null) {
            ...
            if (!r.activity.mFinished && willBeVisible && r.activity.mDecor != null && !r.hideForNow) {
                if (r.newConfig != null) {
                    ...
                    if (DEBUG_CONFIGURATION)
                        Slog.v(TAG, "Resuming activity " + r.activityInfo.name + " with newConfig " + r.tmpConfig);
    ...
    final Bundle performPauseActivity(ActivityClientRecord r, boolean finished, boolean saveState) {
        ...
        try {
            ...
            if (!mInstrumentation.onException(r.activity, e)) {
                throw new RuntimeException(
                        "Unable to pause activity " + r.intent.getComponent().toShortString() + ": " + e.toString(), e);
            }
        }
        r.paused = true;
    ...
    private void performStopActivityInner(ActivityClientRecord r, StopInfo info, boolean keepShown, boolean saveState) {
        ...
        if (r != null) {
            if (!keepShown && r.stopped) {
                ...
                RuntimeException e = new RuntimeException(
                        "Performing stop of activity that is not resumed: " + r.intent.getComponent().toShortString());
                Slog.e(TAG, e.getMessage(), e);
            }
        ...
        if (v != null) {
            if (show) {
                ...
                if (r.newConfig != null) {
                    ...
                    if (DEBUG_CONFIGURATION)
                        Slog.v(TAG, "Updating activity vis " + r.activityInfo.name + " with new config " + r.tmpConfig);
                    performConfigurationChanged(r.activity, r.tmpConfig);
                    freeTextLayoutCachesIfNeeded(r.activity.mCurrentConfig.diff(r.tmpConfig));
                    r.newConfig = null;
                }
    ...
    private void updateDefaultDensity() {
        if (mCurDefaultDisplayDpi != Configuration.DENSITY_DPI_UNDEFINED
            ...
            Slog.i(TAG,
                    "Switching default density from " + DisplayMetrics.DENSITY_DEVICE + " to " + mCurDefaultDisplayDpi);
    ...
    private void handleBindApplication(AppBindData data) {
        ...
        /*
         * Initialize the default locale in this process for the reasons we set the time
         * zone.
         */
        ...
        /**
         * For apps targetting SDK Honeycomb or later, we don't allow network usage on
         * the main event loop / UI thread.
         *
        ...
        if (data.instrumentationName != null) {
            ...
            mInstrumentation.init(this, instrContext, appContext, new ComponentName(ii.packageName, ii.name),
